    ...
    return (
      <div className="rounded-2xl bg-cream p-7 border border-border shadow-[0_20px_50px_-30px_rgba(60,80,70,0.4)]">
        ...
        <p className="mt-5 text-ink/65 leading-[2] text-[13px]">
          這是預訂流程示意版本，正式網站將會串接第三方 Booking System。
        </p>
        <button onClick={() => setStage("form")} className="mt-5 text-primary text-sm underline underline-offset-4">重新填寫</button>
      </div>
    );
  }
    ...
    >
      <div className="flex items-center justify-between border-b border-border/60 pb-4">
        <div>
          ...
          <h3 className="font-serif text-xl text-ink mt-1 font-semibold">預訂這個行程</h3>
        </div>
        <span className="text-[11px] text-ink/55">起 <span className="text-primary font-serif text-[15px] font-semibold">{tour?.price}</span></span>
      </div>

      <div>
        <label className="block text-[11px] tracking-[0.2em] uppercase text-ink/55 mb-2">選擇日期</label>
      ...
      <div>
        <label className="block text-[11px] tracking-[0.2em] uppercase text-ink/55 mb-2">選擇團別</label>
        <div className="flex flex-wrap gap-1.5">
          {packages.map((p) => (
            <button
              type="button" key={p}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-[11px] tracking-[0.2em] uppercase text-ink/55 mb-2">剩餘名額</label>
          <p className="pt-1.5 text-primary font-serif text-lg font-semibold">{dep.seats} <span className="text-[11px] text-ink/55 font-sans">位</span></p>
        </div>
      </div>
    ...
    <SiteLayout>
      ...
      <div className="mx-auto max-w-[1240px] px-5 md:px-10 -mt-10 md:-mt-16 relative pb-24">
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-10">
          ...
          {/* RIGHT — sticky booking (~35%) — desktop only */}
      ...
      <div className="lg:hidden sticky bottom-0 z-40 bg-cream/95 backdrop-blur border-t border-border px-5 py-3 flex items-center justify-between gap-3 shadow-[0_-10px_30px_-15px_rgba(60,80,70,0.3)]">
        <div>
          <p className="text-[10.5px] text-ink/55 tracking-[0.2em] uppercase">起價</p>
